...
export default async function ExplorePage({ params }: { params: { id: string } }) {
  try {
      ...
      <div className="max-w-[1369px] container mx-auto p-4 mt-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="max-h-[800px] overflow-y-auto">
            <Suspense fallback={<div className="space-y-6">{[...Array(5)].map((_, index) => <ServiceCardSkeleton key={index} />)}</div>}>
    ...
    return (
      <div className="max-w-[1369px] container mx-auto p-4 mt-16">
        <p className="text-red-500">An error occurred: {(error as Error).message}</p>
      </div>
    );
  }
}